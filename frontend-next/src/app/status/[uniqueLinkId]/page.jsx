      ...
      <div className="min-h-screen flex justify-center items-center bg-neutral-950 px-6 relative overflow-hidden transition-colors">
        ...
        <div className="bg-white/5 backdrop-blur-3xl p-10 rounded-[2.5rem] border border-danger-500/20 shadow-2xl text-center max-w-md w-full animate-fade-up z-10">
          ...
          <p className="mt-4 text-white font-medium">
            Your appointment was cancelled. Please ask at the front desk if you
            need help.
          </p>
        </div>
      </div>
    ...
    return (
      <div className="min-h-screen flex justify-center items-center bg-neutral-950 px-6 relative overflow-hidden py-10 transition-colors">
        ...
        <div className="bg-white/5 backdrop-blur-3xl p-8 sm:p-10 rounded-[3rem] border border-success-500/20 shadow-2xl text-center max-w-md w-full animate-fade-up z-10 flex flex-col items-center">
          ...
          <p className="mt-3 text-sm sm:text-base text-white font-bold max-w-[280px] mx-auto uppercase tracking-wide">
    ...
    <div className="min-h-screen bg-neutral-950 text-white selection:bg-brand-500/30 font-sans relative overflow-x-hidden pt-8 pb-20 px-4 sm:px-6 transition-colors">
      ...
      <div className="max-w-md mx-auto relative z-10 space-y-6">
        ...
        <div className="bg-white/5 backdrop-blur-3xl border border-white/10 rounded-[2.5rem] p-8 shadow-2xl text-center animate-fade-down">
          ...
          <p className="text-neutral-100 text-[10px] font-black uppercase tracking-[0.3em] mt-4">
          ...
          {msg === "Tracking Link Copied! 🎉" && (
            <p className="text-[10px] text-brand-300 mt-3 font-bold animate-fade-up">
              Tracking link saved to clipboard!
            </p>
          )}
        </div>
        ...
        <div className="bg-white/5 backdrop-blur-3xl border border-white/10 rounded-[2.5rem] p-6 sm:p-8 shadow-xl space-y-8">
          <div className="flex justify-between items-start px-2 relative">
            ...
            <div className="flex flex-col items-center gap-2 sm:gap-3 z-10 group">
              <div
                className={`w-9 h-9 sm:w-11 sm:h-11 rounded-xl flex items-center justify-center border-2 transition-all ${data.myPosition > 0 ? "bg-brand-500 border-brand-400 shadow-[0_0_15px_rgba(59,130,246,0.3)]" : "bg-neutral-900 border-neutral-800"}`}
              >
                <CheckCircle className="w-4 h-4 sm:w-5 sm:h-5 text-white" />
              </div>
              <span className="text-[10px] sm:text-[11px] font-bold uppercase tracking-wider text-neutral-100">
                Check-in
              </span>
            </div>
            ...
            <div className="flex flex-col items-center gap-2 sm:gap-3 z-10 group">
              <div
                className={`w-9 h-9 sm:w-11 sm:h-11 rounded-xl flex items-center justify-center border-2 transition-all ${data.myPosition > 3 ? "bg-brand-500 border-brand-400 shadow-[0_0_15px_rgba(59,130,246,0.3)] animate-pulse" : data.myPosition > 0 ? "bg-brand-500 border-brand-400" : "bg-neutral-900 border-neutral-800"}`}
              >
            ...
            <div className="flex flex-col items-center gap-2 sm:gap-3 z-10 group">
              <div
                className={`w-9 h-9 sm:w-11 sm:h-11 rounded-xl flex items-center justify-center border-2 transition-all ${data.myPosition <= 3 && data.myPosition > 0 ? "bg-brand-500 border-brand-400 shadow-[0_0_15px_rgba(59,130,246,0.3)] animate-bounce" : "bg-neutral-900 border-neutral-800"}`}
              >
          ...
          <div className="grid grid-cols-2 gap-3 sm:gap-4">
            <div className="bg-white/[0.02] border border-white/5 rounded-[1.5rem] sm:rounded-[2rem] p-4 sm:p-6 text-center group hover:bg-white/5 transition-all shadow-inner">
              <p className="text-neutral-100 text-[11px] font-bold uppercase tracking-wider">
                NUMBER
              </p>
              <p className="text-3xl sm:text-4xl font-black text-white mt-1 sm:mt-2 group-hover:scale-110 transition-transform">
                #{data.myTokenNumber}
              </p>
            </div>
            <div className="bg-brand-500/10 border border-brand-500/20 rounded-[1.5rem] sm:rounded-[2rem] p-4 sm:p-6 text-center group hover:bg-brand-500/20 transition-all shadow-inner">
              <p className="text-brand-300 text-[11px] font-bold uppercase tracking-wider">
                PLACE IN LINE
              </p>
              <p className="text-3xl sm:text-4xl font-black text-white mt-1 sm:mt-2 group-hover:scale-110 transition-transform">
                {data.myPosition}
            ...
            <div className="bg-white/[0.02] border border-white/10 rounded-[2rem] p-5 flex items-center justify-between shadow-inner group hover:bg-white/5 transition-all">
              <div className="flex items-center gap-4">
                <div className="w-12 h-12 bg-neutral-900 rounded-2xl flex items-center justify-center border border-white/10 relative overflow-hidden group-hover:border-brand-500/50 transition-colors">
                  <Activity
                    className={`w-6 h-6 z-10 ${agentStatus === "Available" ? "text-brand-400" : "text-neutral-600"}`}
                  />
                ...
                <div>
                  <p className="text-[10px] font-bold text-neutral-100 uppercase tracking-wider">
                    AGENT STATUS
                  </p>
                  <p
                    className={`text-[13px] font-black tracking-wide ${agentStatus === "Available" ? "text-brand-400" : "text-danger-500 shadow-[0_0_10px_rgba(239, 68, 68, 0.3)]"}`}
                  >
                    {agentStatus === "Available"
                      ? "AGENT IS ONLINE"
                      : "AGENT IS OFFLINE"}
            ...
            {data.myPosition <= 3 && agentStatus === "Available" && (
              <div className="bg-brand-600 border border-brand-400 p-5 rounded-[2rem] flex items-center gap-4 animate-pulse shadow-xl shadow-brand-600/20">
                ...
                <p className="text-[11px] font-bold text-white uppercase tracking-wider leading-relaxed">
                  It's almost your turn! Please wait near the office.
                </p>
              </div>
            )}
          </div>
        ...
        <div className="bg-white/5 backdrop-blur-3xl border border-white/10 rounded-[2.5rem] p-8 shadow-xl">
          <div className="flex items-center justify-between mb-8">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-[11px] font-bold uppercase tracking-wider text-neutral-100">
            ...
            {data.queue.map((p) => (
              ...
              >
                <div className="flex items-center gap-5">
                  <span
                    className={`text-base font-black ${p.isMe ? "text-white" : "text-neutral-100"}`}
                  >
                    #{p.tokenNumber}
                  </span>
                  <div
                    className={`w-1.5 h-1.5 rounded-full ${p.isMe ? "bg-white shadow-[0_0_8px_white]" : "bg-neutral-800"}`}
                  />
                  <span
                    className={`text-[11px] font-bold uppercase tracking-wider ${p.isMe ? "text-brand-100" : "text-neutral-100"}`}
                  >
                    {p.isMe ? "THIS IS YOU" : "MEMBER"}
                  </span>
                </div>
                <div
                  className={`px-4 py-1.5 rounded-xl text-[10px] font-bold border uppercase tracking-wider transition-all
                                    ${
                                      p.isMe

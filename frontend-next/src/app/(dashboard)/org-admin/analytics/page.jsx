...
export default function AnalyticsDashboard() {
  ...
  const getHeatmapColor = (count) => {
    ...
    if (intensity < 0.5)
      return "bg-brand-500/30 border-brand-500/40 shadow-[0_0_15px_rgba(59,130,246,0.2)]";
    if (intensity < 0.8)
      return "bg-brand-500/60 border-brand-500/60 text-white shadow-[0_0_20px_rgba(59,130,246,0.4)]";
    return "bg-brand-500 border-white/20 text-white font-bold shadow-[0_0_30px_rgba(59,130,246,0.6)] animate-pulse";
  };

  const totalCustomers = agentPerformance.reduce(
    (acc, doc) => acc + doc.total,
    0,
  );
  const totalCompleted = agentPerformance.reduce(
    (acc, doc) => acc + (doc.completed || 0),
    0,
  );
  const totalCancelled = agentPerformance.reduce(
    (acc, doc) => acc + (doc.cancelled || 0),
    0,
  );
  const completedRate = totalCustomers
    ? Math.round((totalCompleted / totalCustomers) * 100)
    : 0;

  const validWaitTimes = agentPerformance
    .filter((d) => d.avgWaitTime != null)
    .map((d) => d.avgWaitTime);
  const avgOverallWait = validWaitTimes.length
    ? Math.round(
      ...
      <div className="max-w-7xl mx-auto px-6 sm:px-10 pt-10 pb-20 relative z-10">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12 animate-fade-down">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-xl sm:text-3xl font-black tracking-tight text-white mb-1 uppercase tracking-tighter italic underline decoration-brand-500/20 underline-offset-4">
                Node Analytics
              </h1>
              <p className="text-neutral-500 text-[10px] font-black uppercase tracking-[0.4em]">
                Matrix Throughput{" "}
                <span className="mx-2 text-neutral-800">/</span> 30-Day
                Intelligence
              </p>
            </div>
          </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-12 animate-fade-up">
          {[
            ...
            },
            {
              ...
              color: "text-success-400 border-success-500/20",
            },
            {
              label: "Neural Latency",
              value: `${avgOverallWait}m`,
              icon: <Zap className="w-5 h-5" />,
              color: "text-warning-400 border-warning-500/20",
            },
            {
              label: "Sync Failures",
              value: totalCancelled,
              icon: <XCircle className="w-5 h-5" />,
              color: "text-danger-400 border-danger-500/20",
            },
          ].map(({ label, value, icon, color }) => (
            <div
              key={label}
              className={`group relative bg-white/[0.03] border rounded-[1.5rem] sm:rounded-2xl p-5 sm:p-7 transition-all hover:bg-white/[0.05] flex flex-col items-center text-center backdrop-blur-xl ${color}`}
            >
              <div className="absolute top-4 right-4 sm:top-5 sm:right-5 opacity-30 group-hover:opacity-100 transition-opacity">
                {icon}
              </div>
              <span className="text-2xl sm:text-4xl font-black tracking-tighter mb-1 font-mono whitespace-nowrap">
                {value}
              </span>
              <p className="text-[8px] sm:text-[9px] uppercase font-black tracking-[0.2em] text-neutral-600">
                {label}
              </p>
            </div>
          ))}
        ...
        <div className="grid lg:grid-cols-2 gap-10 mb-10 animate-fade-up delay-100">
          ...
          <div className="bg-white/5 border border-white/10 rounded-[2.5rem] p-6 sm:p-8 backdrop-blur-3xl relative overflow-hidden group">
            ...
            <div className="flex items-center justify-between mb-8">
              ...
              <span className="text-[7px] font-black text-neutral-600 uppercase tracking-widest bg-white/5 px-2 py-0.5 rounded-full border border-white/5">
                Auto-Scaling Axis
              </span>
            </div>
          ...
          <div className="flex items-center gap-4 mb-10">
            ...
            <div>
              ...
              <p className="text-[9px] font-black text-neutral-600 uppercase tracking-widest">
                Global Traffic Node Distribution
              </p>
            </div>
          </div>
            ...
            <div className="min-w-[800px] pb-6">
              ...
              <div className="flex ml-20 mb-6">
                {hours.map((h) => (
                  <div
                    key={h}
                    className="flex-1 text-center text-[10px] font-black text-neutral-600 uppercase tracking-widest italic"
                  >
              ...
              <div className="flex flex-col gap-3">
                {days.map((dayName, dayIndex) => (
                  <div key={dayName} className="flex items-center group/row">
                    <div className="w-20 text-[10px] font-black text-neutral-700 text-right pr-6 group-hover/row:text-white transition-colors duration-500 uppercase tracking-[0.2em] italic">
                      {dayName}
                    </div>
                    <div className="flex-1 flex gap-2">
                      {hours.map((hour) => {
                        const point = heatmap.find(
                          (h) =>
                            h.dayOfWeek === dayIndex && h.hourOfDay === hour,
                        );

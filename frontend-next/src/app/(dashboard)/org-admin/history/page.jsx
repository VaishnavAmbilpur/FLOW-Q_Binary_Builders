...
export default function CustomerHistory() {
  ...
  const loadAgents = async () => {
    try {
      ...
      setAgents(res.data.filter((s) => s.role === "AGENT"));
    } catch (err) {
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-neutral-500 text-[10px] font-black uppercase tracking-[0.4em]">
                Visit Logs <span className="mx-2 text-neutral-800">/</span>{" "}
                Records
              </p>
            </div>
          </div>
              ...
              <table className="w-full text-left border-separate border-spacing-y-4 px-6 md:px-10">
                ...
                <tbody>
                  {history.map((customer) => (
                    <tr
                      key={customer._id}
                      className="group transition-all duration-500 hover:z-20 relative"
                    >
                      <td className="bg-white/[0.02] border-y border-l border-white/5 p-5 rounded-l-2xl group-hover:bg-white/[0.06] group-hover:border-white/20 transition-all duration-500">
                        <div className="flex items-center gap-4 text-neutral-300">
                          ...
                          <div>
                            <h4 className="text-base font-black tracking-tight group-hover:text-brand-400 transition-colors uppercase">
                              {customer.name}
                            </h4>
                            <p className="text-[9px] font-black text-neutral-600 uppercase tracking-widest mt-0.5">
                              Visit ID {customer._id.slice(-6).toUpperCase()}
                            </p>
                          </div>
                        </div>
                      </td>
                      ...
                      <td className="bg-white/[0.02] border-y border-white/5 p-5 group-hover:bg-white/[0.06] group-hover:border-white/20 transition-all duration-500">
                        <div className="flex items-center gap-2 group-hover:gap-4 text-neutral-400 group-hover:text-white transition-all duration-300">
                          ...
                          <div>
                            <p className="text-[9px] font-black uppercase tracking-tight italic">
                              {customer.agentId?.name}
                            </p>
                            <p className="text-[7px] font-black text-neutral-800 uppercase tracking-widest">
                              {customer.agentId?.serviceCategory}
                            </p>
                          </div>
                        </div>
                      </td>
                        ...
                        <div className="flex flex-col gap-1">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-[10px] font-black uppercase tabular-nums">
                              {new Date(
                                customer.completedAt || customer.updatedAt,
                              ).toLocaleDateString()}
                            </span>
                          </div>
                          <div className="flex items-center gap-2">
                      ...
                      <td className="bg-white/[0.02] border-y border-r border-white/5 p-8 rounded-r-[2rem] group-hover:bg-white/[0.06] group-hover:border-white/20 transition-all duration-500 text-right pr-10">
                        <span className="text-4xl font-black font-mono tracking-tighter text-white/20 group-hover:text-brand-500 transition-colors">
                          {customer.tokenNumber.toString().padStart(2, "0")}
                        </span>
                      </td>
                    </tr>
                  ))}
        ...
        {!loading && history.length > 0 && (
          <div className="mt-12 flex items-center justify-between px-10 animate-fade-up animate-delay-300">
            <div className="flex items-center gap-4">
              ...
              <p className="text-[10px] font-black text-neutral-700 uppercase tracking-[0.5em]">

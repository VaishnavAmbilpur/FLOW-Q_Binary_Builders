      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 md:px-10 py-4 md:py-6 relative z-10">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-8 animate-fade-down relative z-20">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-xl sm:text-3xl font-black tracking-tight text-white mb-0.5 uppercase italic">
                Registry Desk
              </h1>
              <p className="text-neutral-500 text-[11px] font-bold uppercase tracking-wider">
                Customer Enrollment Terminal{" "}
                <span className="mx-2 text-white/10">|</span>{" "}
                {organization?.name || "Standard Registry"}
              </p>
            </div>
          </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 mb-8 animate-fade-up">
          {[
            ...
            },
            {
              ...
              color: "text-info-400 bg-info-500/5 border-info-500/10",
            },
            {
              label: "Finalized",
              value: completedCount,
              icon: <CheckCircle className="w-5 h-5" />,
              color: "text-success-400 bg-success-500/5 border-success-500/10",
            },
            {
              label: "Active Agents",
              value: agents.length,
              icon: <Briefcase className="w-5 h-5" />,
              color: "text-warning-400 bg-warning-500/5 border-warning-500/10",
            },
          ].map(({ label, value, icon, color }) => (
            <div
              key={label}
              className={`group relative rounded-2xl border shadow-sm p-5 transition-all hover:bg-white/[0.05] flex flex-col items-center text-center backdrop-blur-xl ${color}`}
            >
              <div className="absolute top-4 right-4 opacity-40 group-hover:opacity-100 transition-opacity hidden sm:block">
                {icon}
              </div>
              <span className="text-3xl font-black tracking-tighter mb-1 font-mono">
            ...
            {tab === "add" && activeTab === "walkin" && (
              <div className="max-w-4xl mx-auto animate-fade-down py-10">
                <div className="text-center mb-12">
                  ...
                  <p className="text-neutral-500 text-[12px] font-bold uppercase tracking-wider">
                    ...
                    >
                      ...
                      {agents.map((a) => {
                        ...
                        return (
                          ...
                          >
                            {a.name} ({a.serviceCategory || a.role}){" "}
                            {isOff ? " [OFFLINE]" : ""}
                          </option>
                        );
                      })}
            ...
            {tab === "add" && activeTab === "appointment" && (
              <div className="max-w-4xl mx-auto animate-fade-up py-10">
                <div className="text-center mb-12">
                  ...
                  <p className="text-neutral-500 text-[11px] font-black uppercase tracking-[0.4em]">
                    Future Schedule Protocol
                  </p>
                </div>
                <form
                ...
                <div className="flex items-center gap-3 mb-10">
                  ...
                  <div className="ml-auto flex items-center gap-4">
                    {[
                      ...
                      >
                        <span
                          className={`text-[10px] font-black uppercase tracking-widest ${m.color}`}
                        >
                          {m.label}
                        </span>
                        <span className="font-mono font-black text-white">
                          {m.count.toString().padStart(2, "0")}
                        </span>
                      </div>
                    ))}
                  </div>
                        ...
                        >
                          {queue.map((p) => (
                            <SortableItem
                              ...
                              agentStatus={
                                availabilities[p.agentId?._id || p.agentId]
                              }
                              onCopyPhone={() => {
                  ...
                  <div className="lg:col-span-5 space-y-4">
                    <div className="flex items-center justify-between mb-2 px-6">
                      ...
                      <button
                        ...
                        className="text-[10px] font-black text-brand-400 uppercase tracking-widest hover:underline"
                      >
                        Refresh
                      </button>
                    </div>
                            ...
                            >
                              <div className="flex items-center gap-5">
                                ...
                                <div className="flex-1 min-w-0">
                                  <p className="text-sm font-black text-white italic uppercase tracking-tight truncate group-hover:text-brand-400 transition-colors">
                                    {a.customerName ||
                                      a.customerName ||
                                      a.guestName}
                                  </p>
...
function FormInput({ label, icon, children, fullWidth = false }) {
  return (
...
function SortableItem({
  ...
  const {
    ...
    data: {
      agentId:
        item.agentId?._id || item.agentId || item.agentId?._id || item.agentId,
    },
  });
  const style = {
  ...
  return (
    ...
    >
      <div
        {...listeners}
        {...attributes}
        className="cursor-grab active:cursor-grabbing p-2 text-neutral-600 hover:text-white transition-all"
      >
      ...
      <div className="flex-1 min-w-0">
        <h4 className="text-lg font-black text-white truncate mb-1 group-hover:text-brand-400 transition-colors uppercase tracking-tight italic">
        ...
        <div className="flex items-center gap-5">
          ...
          <span
            className={`flex items-center gap-1.5 text-[11px] font-bold uppercase tracking-wider ${agentStatus?.availability === "Not Available" || agentStatus?.availability === "Unavailable" ? "text-danger-400" : "text-neutral-500"} group-hover:text-brand-400 transition-colors italic`}
          >
            <Briefcase className="w-3.5 h-3.5" /> Assigned to:{" "}
            {item.agentId?.name || item.agentName || "Standard Agent"}
            {(agentStatus?.availability === "Not Available" ||
              agentStatus?.availability === "Unavailable") && (
              <span className="ml-2 px-1.5 py-0.5 bg-danger-500/10 border border-danger-500/20 rounded text-[7px] animate-pulse">
                OFFLINE
              </span>
            )}
          </span>

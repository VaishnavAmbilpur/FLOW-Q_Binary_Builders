...
export default function DeveloperPortal() {
  ...
  const generateKey = async () => {
    ...
    try {
      const res = await api.post("/organizations/keys", {
        name: keyName,
        isLive: true,
      });
  ...
  return (
    <div className="w-full max-w-6xl mx-auto px-6 pb-20 pt-8 animate-fadeIn">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 gap-4">
        <div>
          ...
          <p className="text-gray-500 dark:text-gray-400 mt-2 font-medium">
            Manage your B2B API Keys and Webhook subscriptions
          </p>
        </div>
        <div className="flex gap-4 items-center">
          <a
            href="/docs"
            target="_blank"
            className="text-sm font-bold text-blue-600 dark:text-cyan-400 hover:underline"
          >
            View API Docs ↗
          </a>
        </div>
      ...
      {msg.text && (
        <div
          className={`mb-6 p-4 rounded-xl border flex items-center gap-3 animate-fadeIn font-semibold ${msg.type === "error" ? "bg-red-50 dark:bg-red-900/20 text-red-600 border-red-200 dark:border-red-900/30" : "bg-emerald-50 dark:bg-emerald-900/20 text-emerald-600 border-emerald-200 dark:border-emerald-900/30"}`}
        >
          {msg.type === "error" ? (
      ...
      <div className="mb-8 border border-black/5 dark:border-white/10 bg-white/80 dark:bg-white/5 backdrop-blur-xl rounded-3xl p-8 shadow-xl relative overflow-hidden">
        ...
        <div className="flex justify-between items-center mb-6">
          <h3 className="text-xl font-bold text-gray-900 dark:text-white flex items-center gap-3">
            ...
            <span className="text-xs px-2 py-1 bg-emerald-100 dark:bg-emerald-500/20 text-emerald-700 dark:text-emerald-400 rounded-full font-mono uppercase tracking-widest">
              {usage.plan} Plan
            </span>
          </h3>
        </div>
        ...
        <div className="relative pt-1">
          <div className="flex mb-2 items-center justify-between text-sm">
            ...
            <div className="font-bold text-emerald-600 dark:text-emerald-400">
              {usage.limit === null
                ? "Unlimited"
                : `${usage.limit.toLocaleString()} Quota`}
            </div>
          </div>
          {usage.limit !== null && (
            <div className="overflow-hidden h-3 mb-4 text-xs flex rounded-full bg-emerald-100 dark:bg-emerald-900/30">
              <div
                ...
                className={`shadow-none flex flex-col text-center whitespace-nowrap text-white justify-center ${usage.used >= usage.limit ? "bg-red-500" : "bg-emerald-500"} transition-all duration-1000`}
              ></div>
            </div>
          )}
        </div>
        ...
        <div className="space-y-6">
          <div className="border border-black/5 dark:border-white/10 bg-white/80 dark:bg-[#0b1021] backdrop-blur-xl rounded-3xl p-8 shadow-xl relative overflow-hidden">
            ...
            {newKey && (
              <div className="mb-8 p-6 bg-red-50 dark:bg-red-900/20 border-2 border-red-200 dark:border-red-500/30 rounded-2xl animate-fadeIn">
                <h4 className="font-bold text-red-700 dark:text-red-400 flex items-center gap-2 mb-2">
                  <ShieldCheck className="w-5 h-5" /> Save this key now!
                </h4>
                <p className="text-sm text-red-600 dark:text-red-300 mb-4">
                  This key will only be displayed once. If you lose it, you will
                ...
                <div className="flex items-center gap-2 bg-white dark:bg-black/50 p-3 rounded-lg border border-red-200 dark:border-red-500/20">
                  <code className="flex-1 font-mono text-sm text-gray-900 dark:text-gray-200 break-all">
                    {newKey}
                  </code>
                  <button
            ...
            <div className="space-y-4">
              <h4 className="text-sm font-bold text-gray-500 dark:text-gray-400 uppercase tracking-widest">
                Active Keys
              </h4>
              {keys.length === 0 ? (
                ...
                keys.map((k) => (
                  <div
                    key={k._id}
                    className="flex items-center justify-between p-4 bg-gray-50 dark:bg-white/5 rounded-xl border border-gray-100 dark:border-white/5"
                  >
                    <div>
                      ...
                      <div className="flex items-center gap-2 mt-1">
                        <span className="text-xs px-2 py-0.5 bg-cyan-100 dark:bg-cyan-500/20 text-cyan-700 dark:text-cyan-400 rounded-full font-mono">
                          {k.prefix}
                        </span>
        ...
        <div className="space-y-6">
          <div className="border border-black/5 dark:border-white/10 bg-white/80 dark:bg-[#110c21] backdrop-blur-xl rounded-3xl p-8 shadow-xl relative overflow-hidden">
            ...
            <p className="text-sm text-gray-600 dark:text-gray-400 mb-6">
              Register an endpoint URL to receive real-time POST requests when
              queue events happen.
            </p>
                ...
                webhooks.map((w) => (
                  ...
                  >
                    <div className="overflow-hidden w-[80%]">
                      ...
                      <p className="text-xs text-purple-600 dark:text-purple-400 mt-1 font-semibold truncate">

...
export default function TeamManagement() {
  ...
  const provisionAgent = async (e) => {
    ...
    try {
      ...
      showMsg(
        err.response?.data?.message || "Failed to provision agent",
        "error",
      );
    } finally {
      setLoading(false);
    ...
    <div className="w-full max-w-6xl mx-auto px-6 pb-20 pt-8 animate-fadeIn">
      <div className="mb-10">
        ...
        <p className="text-gray-500 dark:text-gray-400 mt-2 font-medium">
          Provision and manage hubal accounts for your organization
        </p>
      </div>
          ...
          <form onSubmit={provisionAgent} className="space-y-4 relative z-10">
            <div>
              ...
              <input
                ...
                placeholder="Professional Sarah Connor"
                className="w-full px-4 py-3 rounded-xl bg-gray-50 dark:bg-white/5 border border-gray-200 dark:border-white/10 focus:ring-2 focus:ring-cyan-500 outline-none text-gray-900 dark:text-white"
              />
            </div>
            <div>
              <label className="block text-xs font-bold text-gray-500 uppercase mb-1 ml-1">
            ...
            <button
              disabled={loading}
              type="submit"
              className="w-full py-3 mt-4 bg-cyan-500 hover:bg-cyan-600 text-white font-bold rounded-xl flex items-center justify-center gap-2 transition-colors shadow-lg shadow-cyan-500/20 disabled:opacity-50"
            >
          ...
          {agents.length === 0 ? (
            ...
            <div className="space-y-4 relative z-10">
              {agents.map((doc) => (
                <div
                  key={doc._id}
                  className="flex items-center justify-between p-5 bg-gray-50 dark:bg-white/5 rounded-2xl border border-gray-100 dark:border-white/5 hover:border-cyan-500/30 transition-colors"
                >
                  <div className="flex items-start gap-4">
                    ...
                    <div>
                      ...
                      <div className="flex gap-2 mt-2">
                        <span className="text-xs px-2 py-0.5 bg-purple-100 dark:bg-purple-500/20 text-purple-700 dark:text-purple-400 rounded-full font-semibold">
                          {doc.serviceCategory}
                        </span>
                        <span
                          className={`text-xs px-2 py-0.5 rounded-full font-semibold ${doc.availability === "Available" ? "bg-emerald-100 dark:bg-emerald-500/20 text-emerald-700 dark:text-emerald-400" : "bg-gray-200 dark:bg-gray-800 text-gray-600 dark:text-gray-400"}`}
                        >

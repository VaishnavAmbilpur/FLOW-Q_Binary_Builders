...
export default function KioskDisplayMode() {
  ...
  const fetchDisplayData = async () => {
    try {
      const res = await fetch(
        `${NEXT_PUBLIC_API_URL}/kiosk/${organizationId}/display`,
      );
  ...
  return (
    <div
      className="min-h-screen bg-gray-50 dark:bg-[#060c21] text-gray-900 dark:text-white p-8 md:p-12 relative flex flex-col overflow-hidden transition-colors"
    ...
    >
      ...
      {/* Header */}
      <header className="flex items-center justify-between mb-12 relative z-10 bg-white/90 dark:bg-neutral-900/40 backdrop-blur-xl border border-gray-200 dark:border-white/10 rounded-3xl p-6 shadow-xl dark:shadow-2xl">
      ...
      <main className="flex-1 relative z-10">
        {agents.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full text-center">
            ...
            <h2 className="text-4xl font-bold text-neutral-400">
              No active lists right now
            </h2>
          </div>
        ) : (
            ...
            {agents.map((doc) => (
              ...
              >
                <div className="border-b border-gray-200 dark:border-white/10 pb-6 mb-6">
                  ...
                  <p className="text-xl text-gray-700 dark:text-neutral-300 font-semibold">
                    {doc.serviceCategory}
                  </p>
                </div>
                ...
                <div className="flex-1 flex flex-col justify-center items-center py-6 bg-gray-50 dark:bg-black/40 rounded-3xl mb-8 border border-gray-200 dark:border-white/5 relative overflow-hidden">
                  ...
                  <p
                    className={`text-[7rem] leading-none font-black tracking-tight relative z-10 drop-shadow-sm dark:drop-shadow-[0_0_20px_rgba(255,255,255,0.3)] ${doc.servingToken === "---" ? "text-gray-400 dark:text-neutral-600" : "text-gray-900 dark:text-white"}`}
                  >
                    {doc.servingToken}
                  </p>
                </div>

            ...
            <header className="flex items-center justify-between mb-16 relative z-10 border-b border-white/5 pb-12">
                ...
                <div className="flex items-center gap-8 bg-white/5 p-6 rounded-[2.5rem] border border-white/10 backdrop-blur-md">
                    <div className="text-right">
                        <p className="text-[10px] font-black text-neutral-500 uppercase tracking-[0.4em] mb-1">Standard Time</p>
                ...
                {doctors.length === 0 ? (
                    ...
                    doctors.map((doc: any, index: number) => (
                        ...
                        >
                            ...
                            <div className={`relative rounded-[3rem] p-12 border transition-all duration-700 shadow-2xl overflow-hidden ${doc.servingToken !== '---' ? 'bg-white text-black border-white' : 'bg-black/40 border-white/5 text-neutral-700'}`}>
                                {doc.servingToken !== '---' && (
                                    <div className="absolute inset-0 bg-gradient-to-br from-brand-400/20 to-transparent pointer-events-none opacity-50" />
                                )}
                                <p className={`text-[10px] font-black uppercase tracking-[0.5em] mb-4 text-center ${doc.servingToken !== '---' ? 'text-black/40' : 'text-neutral-800'}`}>Currently Serving</p>
                                <div className="flex items-center justify-center">
                                    <span className={`text-[11rem] font-black leading-none tracking-tighter font-mono drop-shadow-2xl transition-all duration-700 ${doc.servingToken !== '---' ? 'scale-105' : 'scale-100 opacity-20'}`}>
                                        {doc.servingToken}
                                    </span>
                                </div>
                            </div>
                    ...
                    <div className="whitespace-nowrap flex items-center gap-16 animate-marquee">
                        {[1, 2, 3].map((i) => (
                            <React.Fragment key={i}>
                                <span className="text-neutral-500 text-xl font-black uppercase tracking-widest flex items-center gap-4"><ArrowRight className="w-6 h-6 text-brand-400" /> Watch for your number to pulsate on the matrix.</span>
                                <span className="text-neutral-500 text-xl font-black uppercase tracking-widest flex items-center gap-4"><ArrowRight className="w-6 h-6 text-brand-400" /> Self-service nodes bridge the check-in gap instantly.</span>
                                <span className="text-neutral-500 text-xl font-black uppercase tracking-widest flex items-center gap-4"><ArrowRight className="w-6 h-6 text-brand-400" /> Wait times are approximate and reflect current status.</span>
                            </React.Fragment>
                        ))}
                    </div>
                </div>

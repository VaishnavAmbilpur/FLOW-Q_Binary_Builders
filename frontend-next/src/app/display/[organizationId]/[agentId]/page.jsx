    ...
    <div className="min-h-screen bg-[#050505] text-white font-sans relative overflow-hidden flex flex-col p-12 transition-all">
      ...
      <header className="flex items-center justify-between mb-16 relative z-10 border-b border-white/5 pb-12">
        <div className="flex items-center gap-8">
          ...
          <div>
            <h1 className="text-3xl sm:text-5xl font-black tracking-tighter text-white mb-1 uppercase italic underline decoration-brand-500/20 underline-offset-4">
              Live Queue Stream
            </h1>
            <p className="text-[9px] font-black text-neutral-500 uppercase tracking-[0.4em]">
              Department Display{" "}
              <span className="mx-3 text-neutral-800">/</span>{" "}
              {agentData.organizationName}
            </p>
          </div>
        </div>
      ...
      <main className="flex-1 relative z-10 flex flex-col lg:flex-row gap-16">
        ...
        <div className="flex-[2] flex flex-col justify-center">
          <div className="flex items-center gap-10 mb-20 animate-fade-down">
            ...
            <div>
              <p className="text-[9px] font-black text-brand-400 uppercase tracking-[0.4em] mb-2">
                Operating Hubian
              </p>
              <h2 className="text-5xl sm:text-6xl font-black tracking-tighter uppercase mb-3 italic">
                {agentData.agentName}
              </h2>
              <div className="flex items-center gap-4">
                <span className="px-6 py-2 bg-white/5 border border-white/10 rounded-full text-[10px] font-black uppercase tracking-widest text-neutral-400">
                  {agentData.serviceCategory}
                </span>
          ...
          <div className="relative group animate-fade-up">
            ...
            <div className="relative bg-white/[0.02] border border-white/5 rounded-[5rem] p-24 text-center backdrop-blur-3xl shadow-[0_0_100px_rgba(0,0,0,0.4)]">
              <p className="text-[10px] font-black uppercase tracking-[0.6em] text-neutral-700 mb-6">
                Currently Serving
              </p>
              <span
                className={`text-[14rem] md:text-[18rem] lg:text-[20rem] leading-none font-black tracking-tighter font-mono drop-shadow-[0_0_50px_rgba(255,255,255,0.1)] transition-all duration-1000 ${agentData.servingToken !== "---" ? "text-white" : "text-neutral-900"}`}
              >
        ...
        <div className="flex-[1] flex flex-col bg-white/[0.03] border border-white/5 rounded-[4rem] backdrop-blur-2xl p-12 shadow-2xl relative overflow-hidden animate-fade-left">
          ...
          <div className="flex-1 space-y-6 overflow-y-auto custom-scrollbar pr-2">
            {agentData.nextTokens && agentData.nextTokens.length > 0 ? (
              agentData.nextTokens.map((token, idx) => (
                <div
                  key={idx}
                  className="group bg-white/[0.02] border border-white/5 rounded-[2.5rem] p-8 hover:bg-white/[0.05] hover:border-white/20 transition-all duration-500 flex items-center justify-between"
                >
                  <div>
                  ...
                  <span className="text-4xl md:text-5xl font-black font-mono tracking-tighter text-white group-hover:text-brand-400 transition-colors">
                    {token}
                  </span>
                </div>
              ))
            ...
            {[1, 2].map((i) => (
              <React.Fragment key={i}>
                <span className="text-neutral-500 text-xl font-black uppercase tracking-widest flex items-center gap-4">
                  ...
                  your token number to appear on the screen.
                </span>
                <span className="text-neutral-500 text-xl font-black uppercase tracking-widest flex items-center gap-4">
                  <ArrowRight className="w-6 h-6 text-brand-400" /> Please check
                  in at the reception or kiosk to join the queue.
                </span>
              </React.Fragment>
            ))}
          </div>

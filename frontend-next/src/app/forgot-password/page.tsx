        ...
        <div className="min-h-screen w-full flex items-center justify-center bg-neutral-950 font-sans relative overflow-hidden transition-colors duration-300 selection:bg-brand-500/30 p-6">
            ...
            <div className="relative w-full max-w-[380px] z-10 animate-fade-up">
                ...
                <div className="mb-6 flex justify-center">
                    <Link href="/login" className="inline-flex items-center gap-2 text-neutral-500 hover:text-white transition-colors text-[9px] font-black uppercase tracking-widest bg-white/5 px-3 py-1.5 rounded-full border border-white/10 backdrop-blur-md">
                ...
                <div className="bg-white/[0.03] border border-white/5 backdrop-blur-3xl rounded-[2rem] p-5 sm:p-6 shadow-2xl relative overflow-hidden">
                    ...
                    {isSuccess ? (
                        <div className="rounded-2xl bg-brand-500/10 p-5 border border-brand-500/30 animate-fade-up backdrop-blur-xl">
                            <div className="flex gap-4">
                                ...
                                <div>
                                    <h3 className="text-[10px] font-black text-brand-400 uppercase tracking-widest">Email Dispatched</h3>
                                    <p className="mt-1 text-[11px] text-neutral-400 font-bold leading-relaxed">{message}</p>
                                </div>
                            </div>
                            <Link href="/login" className="mt-6 w-full py-3.5 rounded-xl bg-white text-black font-black text-[9px] uppercase tracking-widest flex items-center justify-center shadow-xl hover:scale-[1.02] transition-all">
                        ...
                        <form className="space-y-4" onSubmit={handleSubmit}>
                            {error && (
                                <div className="rounded-xl bg-danger-500/10 p-4 border border-danger-500/30 animate-shake backdrop-blur-xl">
                                    <p className="text-[10px] font-black text-danger-400 uppercase tracking-widest text-center">{error}</p>
                                </div>
                            )}

                            <div className="space-y-1.5">
                                <label className="text-[8px] font-black text-neutral-600 uppercase tracking-[0.3em] ml-4">Credential Identity</label>
                    ...
                    transition-all active:scale-[0.98] disabled:opacity-50 flex items-center justify-center gap-3"
                            >
                                {isLoading ? (
                                    <>
                                        <Activity className="w-3.5 h-3.5 animate-spin" /> Transmitting...
                                    </>
                                ) : (

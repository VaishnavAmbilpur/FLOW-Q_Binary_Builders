    ...
    switch (user?.role) {
      ...
      case "AGENT":
        return {
          ...
          badge:
            "bg-emerald-100 dark:bg-emerald-900/30 border border-emerald-200 dark:border-emerald-800 text-emerald-600 dark:text-emerald-400",
          icon: Users,
      ...
      case "OPERATOR":
        return {
          ...
          badge:
            "bg-amber-100 dark:bg-amber-900/30 border border-amber-200 dark:border-amber-800 text-amber-600 dark:text-amber-400",
          icon: ClipboardList,
      ...
      default:
        return {
          ...
          badge:
            "bg-blue-100 dark:bg-blue-900/30 border border-blue-200 dark:border-blue-800 text-blue-600 dark:text-blue-400",
          icon: UserCircle,
        ...
        <div className="flex items-center justify-between h-20">
          ...
          >
            ...
            <div className="flex flex-col">
              <span
                className={`font-black text-2xl tracking-tighter bg-gradient-to-r ${roleStyle.gradient} bg-clip-text text-transparent uppercase italic`}
              >
                FLOW-Q
              </span>
              <span className="text-[7px] font-black tracking-[0.4em] text-neutral-100 uppercase -mt-1 opacity-80">
          ...
          <div className="hidden md:flex items-center gap-2 bg-black/5 dark:bg-white/[0.03] p-1.5 rounded-[1.5rem] border border-black/5 dark:border-white/5">
            {navLinks.map((item) => {
              ...
              return (
                ...
                >
                  <Icon
                    className={`w-4 h-4 ${isActive ? "text-black" : "text-neutral-200 group-hover/link:text-brand-400"} transition-colors`}
                  />
            ...
            <div className="relative">
              ...
              >
                <div className="flex flex-col text-right">
                  <div className="text-sm font-black text-neutral-900 dark:text-white uppercase tracking-tighter italic">
                    {user.name}
                  </div>
                ...
                <div
                  className={`w-11 h-11 rounded-[1rem] ${roleStyle.badge} flex items-center justify-center shadow-2xl transform group-hover:-rotate-3 transition-transform`}
                >
              ...
              {userMenuOpen && (
                <div className="absolute right-0 mt-4 w-64 bg-white dark:bg-neutral-950 border border-neutral-200 dark:border-white/10 rounded-[2rem] shadow-2xl overflow-hidden animate-fade-down z-[110]">
                  <div className="p-6 border-b border-neutral-100 dark:border-white/5 bg-neutral-50 dark:bg-white/[0.02]">
                    <div className="text-xs font-black text-neutral-900 dark:text-white uppercase tracking-tight mb-1">
                      {user.name}
                    </div>
                    <div className="text-[10px] text-neutral-200 truncate mb-3">
                      {user.email}
                    </div>
                    <div
                      className={`inline-block px-3 py-1 rounded-full text-[8px] font-black uppercase tracking-[0.2em] border ${roleStyle.badge}`}
                    >
                      {user.role?.replace("_", " ")}
                    </div>
                  </div>

...
// ─── Self-Healing: Drop Legacy Usage Index ──────────────────────────────────
async function selfHeal() {
    try {
        const collection = mongoose.connection.collection("apiusages");
        if (collection) {
            console.log("[V2-API] 🛠️  Checking legacy index 'hospitalId_1_yearMonth_1'...");
            await collection.dropIndex("hospitalId_1_yearMonth_1").catch(() => { });
            console.log("[V2-API] ✅ Legacy index cleanup complete.");
        }
    } catch (e) {
        // Silently skip if already dropped
    }
}
mongoose.connection.on("connected", selfHeal);

// ─── Middleware: Org-Trace Logging ───────────────────────────────────────────
router.use((req, res, next) => {
    if (req.organizationId) {
        console.log(`[V2-API] 🏷️  OrgID: ${req.organizationId} | 📡 ENDPOINT: ${req.method} ${req.originalUrl}`);
    }
    next();
});

// ─── Unauthenticated Sandbox Management ─────────────────────────────────────

/**
 * @swagger
 * /v2/demo/provision:
 *   post:
 *     summary: Provision a new sandbox organization and API key for testing
 *     tags: [SandBox]
 *     responses:
 *       201:
 *         description: New organization and API key created
 */
...
// Every v2 endpoint below requires a valid API key
...
router.post("/queue", async (req, res) => {
    try {
        ...
        res.status(201).json({
            ...
            uniqueLinkId,
            queueEntryId: entry._id,
            statusLink: `/v2/queue/${uniqueLinkId}`,
...
router.get("/queue/:uniqueLinkId", async (req, res) => {
    try {
        ...
        // Calculate position
        const queue = await QueueEntry.find({ serviceId: entry.serviceId, status: entry.status })
            .sort({ tokenNumber: 1 });

        const position = queue.findIndex(p => p._id.toString() === entry._id.toString()) + 1;
        ...
        res.json({
            ...
            data: {
                ...
                estimatedWaitMins: position > 0 ? (position - 1) * (entry.serviceId.avgSessionDuration || 5) : 0,
                createdAt: entry.createdAt,
                uniqueLinkId: entry.uniqueLinkId
            }
        });
    } catch (err) {
        logger.error("v2 GET /queue/:uniqueLinkId error", { error: err.message });
        res.status(500).json({ success: false, message: "Server Error" });
    }
});
...
router.delete("/queue/:uniqueLinkId", async (req, res) => {
    try {
        ...
        logger.error("v2 DELETE /queue/:uniqueLinkId error", { error: err.message });
        res.status(500).json({ success: false, message: "Server Error" });
    }
});
 ...
 * /v2/queue:
 *   get:
 *     summary: List all active queue entries for the organization
 *     tags: [B2B v2]
 *     security:
 *       - ApiKeyAuth: []
 *     parameters:
 ...
 * /v2/queue/check-in:
 *   post:
 *     summary: Direct walk-in check-in to the live queue
 *     tags: [B2B v2]
 *     security:
 *       - ApiKeyAuth: []
 *     requestBody:
...
router.post("/queue/check-in", async (req, res) => {
    try {
        const { serviceId, clientName, clientPhone } = req.body;
        if (!serviceId || !clientName) {
            return res.status(400).json({ success: false, message: "serviceId and clientName are required" });
        }
        ...
        const uniqueLinkId = uuidv4();
        const org = await Organization.findById(req.organizationId).select("locations");
        if (!org) return res.status(404).json({ success: false, message: "Organization resolution failed" });

        const entry = await QueueEntry.create({
            organizationId: req.organizationId,
            locationId: org.locations?.[0]?._id || req.organizationId,
            serviceId: service._id,
            clientName: clientName.trim(),
            clientPhone: (clientPhone || "").trim(),
            tokenNumber,
        ...
        const io = req.app.get("io") || global.io;
        if (io) {
            io.to(`service_${serviceId}`).emit("queue.updated");
            io.to(`org_${req.organizationId}`).emit("queue.updated");
        }
        ...
        logger.error("v2 POST /queue/check-in error", { error: err.message });
        res.status(500).json({ success: false, message: "Server Error" });
    }
});
...
/**
 * @swagger
 * /v2/queue/{uniqueLinkId}/action:
 *   patch:
 *     summary: Perform a merchant action on a queue entry (call/complete/cancel)
 *     tags: [B2B v2]
 *     security:
 *       - ApiKeyAuth: []
 *     requestBody:
 ...
 *               action:
 *                 type: string
 *                 enum: [call, complete, cancel]
 */
router.patch("/queue/:uniqueLinkId/action", async (req, res) => {
    try {
        const { action } = req.body;
        const validActions = ["call", "complete", "cancel"];
        ...
        if (io) {
            io.to(`service_${entry.serviceId}`).emit("queue.updated");
            io.to(`org_${req.organizationId}`).emit("queue.updated");
            io.to(`${entry.uniqueLinkId}`).emit("queue.updated", { status: entry.status });
        }

        dispatchWebhook(req.organizationId, `queue.${action === 'call' ? 'updated' : action === 'complete' ? 'completed' : 'cancelled'}`, {
            queueEntryId: entry._id,
            uniqueLinkId: entry.uniqueLinkId,
            tokenNumber: entry.tokenNumber,
            status: entry.status
        ...
        res.json({ success: true, message: `Queue entry ${action}ed successfully`, status: entry.status });
    } catch (err) {
        logger.error("v2 PATCH /queue/:id/action error", { error: err.message });
        res.status(500).json({ success: false, message: "Server Error" });
    }
});
 ...
 *       - ApiKeyAuth: []
 */
router.patch("/appointments/:id/arrive", async (req, res) => {
    try {
        const policy = await assertAppointmentsEnabledForOrgId(req.organizationId);
        if (!policy.ok) return res.status(policy.status).json({ success: false, message: policy.message });
